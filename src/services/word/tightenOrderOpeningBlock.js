// src/services/word/tightenOrderOpeningBlock.js
...
const setParagraphSpacing = (
  ...
  paragraph,
  {
    ...
    lineTwips = null,
    lineRule = "auto",
    removeKeepNext = false,
    removeKeepLines = false,
    removePageBreakBefore = false,
    removeWidowControl = false,
    removeContextualSpacing = false,
    removeSnapToGrid = false,
  } = {},
) => {
...
const tightenOrderOpeningBlock = (buffer, options = {}) => {
  const {
    nakazuiuText = "НАКАЗУЮ:",
    signatureMarkerText = "__SIGNATURE_START__",
    lineTwips = 240,
    nakazuiuBeforeTwips = 480,
    nakazuiuAfterTwips = 0,
    directiveFirstParagraphBeforeTwips = 480,
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if (nakazuiuIndex === -1 && text === nakazuiuText) {
      nakazuiuIndex = i;
    }
  ...
  for (let i = nakazuiuIndex - 1; i >= 0; i--) {
  ...
  const nakazuiuParagraph = paragraphs[nakazuiuIndex];

  setParagraphSpacing(xmlDoc, nakazuiuParagraph, {
    beforeTwips: nakazuiuBeforeTwips,
    afterTwips: nakazuiuAfterTwips,
    lineTwips,
    lineRule: "auto",
    removeKeepNext: false,
    removeKeepLines: false,
  ...
  let firstDirectiveParagraph = null;

  for (let i = nakazuiuIndex + 1; i < paragraphs.length; i++) {
    const paragraph = paragraphs[i];
    const text = getParagraphText(paragraph);

    if (signatureIndex !== -1 && i >= signatureIndex) {
      break;
    }
    ...
    if (text === signatureMarkerText) {
      break;
    }

    firstDirectiveParagraph = paragraph;
    break;
  }

  if (firstDirectiveParagraph) {
    for (let i = nakazuiuIndex + 1; i < paragraphs.length; i++) {
      const paragraph = paragraphs[i];

      if (paragraph === firstDirectiveParagraph) {
        break;
      }
    ...
    setParagraphSpacing(xmlDoc, firstDirectiveParagraph, {
      beforeTwips: directiveFirstParagraphBeforeTwips,
      afterTwips: directiveFirstParagraphAfterTwips,
      lineTwips,
      lineRule: "auto",
      removeKeepNext: false,
      removeKeepLines: false,
...
module.exports = tightenOrderOpeningBlock;
