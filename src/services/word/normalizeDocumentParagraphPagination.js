...
const findBodyChildIndexByMarkerText = (bodyChildren, markerText) => {
  if (!markerText) return -1;
...
const collectParagraphsBetweenBodyChildIndexes = (
  bodyChildren,
  startIndexInclusive,
  endIndexExclusive,
) => {
  ...
  const startIndex = Math.max(0, startIndexInclusive);
  const endIndex = Math.min(
    Number.isFinite(endIndexExclusive)
      ? endIndexExclusive
      : bodyChildren.length,
    bodyChildren.length,
  );
  ...
  for (let i = startIndex; i < endIndex; i++) {
    result.push(...getAllParagraphs(bodyChildren[i]));
  }
...
const normalizeDocumentParagraphPagination = (buffer, options = {}) => {
  ...
  const stopIndex = findBodyChildIndexByMarkerText(
    bodyChildren,
    stopBeforeMarkerText,
  );

  const startMarkerIndex = findBodyChildIndexByMarkerText(
    bodyChildren,
    startAfterMarkerText,
  );

  const startIndex = startMarkerIndex >= 0 ? startMarkerIndex + 1 : 0;

  const endIndex = stopIndex >= 0 ? stopIndex : bodyChildren.length;

  if (startIndex >= endIndex) {
    return buffer;
  }

  const paragraphs = collectParagraphsBetweenBodyChildIndexes(
    bodyChildren,
    startIndex,
    endIndex,
  );
