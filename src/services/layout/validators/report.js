...
const validateProshuBlockPlacementRules = (pages, markers, violations) => {
  const proshu = normalizeText(markers.proshu || "ПРОШУ:");
  const foundationPhrase = normalizeText(
    markers.foundationPhrase || "На підставі вищезазначеного,",
  );

  if (!proshu || !foundationPhrase) return;

  const proshuOccurrences = findMarkerOccurrences(pages, proshu);
  const foundationOccurrences = findMarkerOccurrences(pages, foundationPhrase);

  for (const proshuOccurrence of proshuOccurrences) {
    const page = pages.find(
      (item) => item.pageNumber === proshuOccurrence.pageNumber,
    );
    if (!page) continue;

    const foundationOnSamePage = foundationOccurrences.find(
      (item) => item.pageNumber === proshuOccurrence.pageNumber,
    );

    if (!foundationOnSamePage) {
      continue;
    }

    if (foundationOnSamePage.lineIndex > proshuOccurrence.lineIndex) {
      continue;
    }

    const meaningfulLinesAfterProshu = page.lines
      .slice(proshuOccurrence.lineIndex + 1)
      .filter((line) => normalizeText(line.text));

    if (meaningfulLinesAfterProshu.length < 3) {
      pushViolation(
        violations,
        "PROSHU_BLOCK_ORPHANED",
        "Блок 'На підставі вищезазначеного, ПРОШУ:' залишився без достатнього продовження на сторінці",
        page.pageNumber,
        {
          linesAfterProshuCount: meaningfulLinesAfterProshu.length,
        },
      );
    }
  }
...
const validateSignatureRules = (pages, markers, violations) => {
  ...
  if (dateHit && nameHit && dateHit.pageNumber !== nameHit.pageNumber) {
    pushViolation(
      violations,
      "SIGN_DATE_DETACHED",
      "Дата підписання відірвана від підпису",
...
const validateReportLayoutRules = (pages, context = {}, violations) => {
  ...
  validateProshuRules(pages, markers, violations);
  validateFoundationPhraseRules(pages, markers, violations);
  validateProshuBlockPlacementRules(pages, markers, violations);
  validateSignatureRules(pages, markers, violations);
};
